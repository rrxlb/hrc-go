...
func RegisterRouletteCommand() *discordgo.ApplicationCommand { return &discordgo.ApplicationCommand{Name: "roulette", Description: "Play a game of Roulette"} }
...
func HandleRouletteCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
    ...
    utils.SendInteractionResponse(s, i, embed, game.buildComponents(), false)
}

func (rg *RouletteGame) buildComponents() []discordgo.MessageComponent {
    if rg.State == "final" { return nil }
...
func HandleRouletteInteraction(s *discordgo.Session, i *discordgo.InteractionCreate) {
    ...
    if strings.HasPrefix(cid, "roulette_bet_") {
        ...
        if betType == "single" { betType = "single_17" }
...
func (rg *RouletteGame) resolveSpin(s *discordgo.Session) {
    ...
    num := rand.Intn(37)
    color := "green"
    if num != 0 { if _, ok := redNumbers[num]; ok { color = "red" } else { color = "black" } }
    rg.ResultNumber = num
    ...
    var xpGain int64
    if profit > 0 { xpGain = profit * utils.XPPerProfit }
    rg.State = "final"
...
func (rg *RouletteGame) totalBet() int64 { var t int64; for _, v := range rg.Bets { t += v }; return t }

func (rg *RouletteGame) calculateProfit() int64 {
    num := rg.ResultNumber
    profit := int64(0)
    if num == 0 {
        for k, v := range rg.Bets { if isEvenMoney(k) { profit -= v/2 } else { profit -= v } }
        return profit
    }
    for k, v := range rg.Bets {
        ...
        mult := int64(1)
        switch {
        case k == "red" && rg.ResultColor == "red": win = true
        case k == "black" && rg.ResultColor == "black": win = true
        case k == "odd" && num%2 == 1: win = true
        case k == "even" && num%2 == 0: win = true
        case k == "1-18" && num >=1 && num <=18: win = true
        case k == "19-36" && num >=19 && num <=36: win = true
        case k == "dozen1" && num >=1 && num <=12: win = true
        case k == "dozen2" && num >=13 && num <=24: win = true
        case k == "dozen3" && num >=25 && num <=36: win = true
        case k == "col1" && num%3 == 1: win = true
        case k == "col2" && num%3 == 2: win = true
        case k == "col3" && num%3 == 0: win = true
        case strings.HasPrefix(k, "single_"):
            parts := strings.Split(k, "_")
            if len(parts)==2 { if n, err := strconv.Atoi(parts[1]); err==nil && n==num { win = true; mult = 35 } }
        }
        if win { profit += v * mult } else { profit -= v }
    }
    return profit
}

func isEvenMoney(k string) bool { return k=="red"||k=="black"||k=="odd"||k=="even"||k=="1-18"||k=="19-36" }
